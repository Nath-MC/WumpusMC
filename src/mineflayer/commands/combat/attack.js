...
module.exports = {
  ...
  execute(client, cmdArgs, eventArgs) {
    if (cmdArgs[0] === "me") {
    ...
    } else if (cmdArgs[0] === "nearest" && cmdArgs[1]) {
      // $attack nearest [searchRadius]
      let searchRadius = parseInt(cmdArgs[1]);

      if (isNaN(searchRadius) || searchRadius < 0) {
        return client.utils.message(client, "Invalid syntax, try using $attack for help.", eventArgs[0]);
      }

      const target = client.nearestEntity(
        (entity) =>
          !["orb", "global", "other", "object", "projectile", "player"].includes(entity.type) &&
          entity.displayName !== "Armor Stand" && // Mojang classifies armor stands as mobs for some reason?
          entity.position.distanceTo(client.player.entity.position) < searchRadius
      );

      client.pvp.attack(target);

      return client.utils.message(client, `Attacking the nearest mob : ${target?.displayName}`, eventArgs[0]);
    } else if (cmdArgs[0] === client.registry.entitiesByName[cmdArgs[0]]?.name) {
      // Si l'argument est un mob valide
      const target = client.nearestEntity((entity) => entity.name == client.registry.entitiesByName[cmdArgs[0]].name); // On cherche si le mob existe
      ...
      return client.utils.message(client, `Attacking the nearest ${target.displayName}`, eventArgs[0]);
    } else if (client.players[cmdArgs[0]]) {
      // a player ?
