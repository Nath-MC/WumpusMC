...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((args) =>
      args
        .setName("username")
        .setDescription("The player's username you want to move at")
    ),
  ...
  async execute(interaction) {
    ...
    if (PlayerCoords) {
      ...
      if (!coords.length === 3) {
        return interaction.editReply({
          embeds: [
            new EmbedBuilder()
              .setTitle("Invalid coordinates were given !")
              .setDescription("```\nX Y Z \ne.g. 100,60,100 or 100 60 100")
              .setColor([255, 0, 0]),
          ],
        });
      }
    ...
    interaction.editReply({
      embeds: [
        new EmbedBuilder()
          .setTitle(`${mcClient.username} is on his way to ${coords.join(" ")}`)
